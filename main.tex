...
Common operations for interacting with the WebAssembly module are copying data to and from the Emscripten heap. For this reason we provide two helper functions: copyToHeap allocates space on the heap and copies bytes from a typed array, and viewHeap returns a typed array which presents a view to underlying bytes on the heap. These functions can be seen in \ref{lst:heap-helpers}.
\begin{listing}[H] 
...
\end{listing} 
Emscripten also offers the getValue function to read a scalar of a specified type stored on the Emscripten heap. For example the function call 'getValue(ptr, 'i32')' reads a 32 bit signed integer from the heap at the memory location given by ptr. We will use this to read pointers off the heap. Pointers default to 32-bit signed integers in WebAssembly.\footnote{TODO: wasm64}


Given all this we can now work with Futhark from JavaScript and do the equivalent of listing \ref{lst:c-api-incr}.
Listing \ref{lst:c-api-incr} showed how to call Futhark's generated C library from the increment.fut example. After compiling the C library to WASM with Emscripten, the library can be called with the JavaScript equivalent shown in \ref{lst:raw_incr}. (TODO: reflect and possible reword)
...
This is basically a one to one match of the C equivalent from listing \ref{lst:c-api-incr}. This is quite cumbersome for a function that simply takes a single integer argument and returns a single integer. 
Listing \ref{lst:c-api} 
...
TODO say this is basically 1-1 match of the C equivalent.

TODO put C snippet and JS snippet that we discuss as being extra bad here so the reader can see it.


Pointers are complicated as they must be tracked on the Emscripten heap. Working with pointers in JavaScript like this is both not idiomatic and not safe. Not only is the JavaScript as tedious as the original C code with respect to manual memory management, it is even worse. This is illustrated in lines 10 to 15. The out\_ptr, which is allocated and freed with \_malloc and \_free, is an additional complication to work around the fact that the result value must be placed into the Emscripten heap. This isn't the case in C because of the shared address space between the caller and the library function. This allows the caller to pass a pointer to a stack allocated result variable. 



This pain point was one of motivating reasons for the design of the API. For Futhark to be practical the implementation details of the raw pointers to the heap should hidden by a layer of abstraction. This is precisely what the API designed in Chapter 4 accomplished. 
