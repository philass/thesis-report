...
Soren: An additional reason why it is important to understand the C API is that the JavaScript API builds on the C API because it is implemented as a wrapper around a compilation of the C backend into WebAssembly and JavaScript glue code. 

Philip: An additional reason why it is important to observe the C API is that the WebAssembly backend builds off the sequential C backend. For this reason our design choices for the API are limited to how we can wrap the C API function calls behind JavaScript classes and functions.
...
\end{verbatim}
This program multiplies each element of a matrix by a scalar. This is most efficiently expressed in Futhark using maps.

\subsection{C API}
When scale.fut is compiled as a C library, the files scale.c and scale.h are generated, being the implementation and header files respectively. Listing TODO is a C program that interfaces with the generated library. 
...
For each entry point function in the library a C function is generated, which takes the futhark context, an output parameter for each type in the result tuple, as well as an input parameter for each argument. The function signature for the generated C function for scale can be seen below.
\begin{verbatim}
...
Python benefits greatly from the ubiquitity of numpy 
