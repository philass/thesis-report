...
The n,p, and m are size variables that are used to enforce the inputs have the correct dimensions, namely that the number of columns of xss equals the number of rows of yss.
...
The approach that many scientific JavaScript libraries take is to accept TypedArrays as arguments, as well as either multiple arguments or an array for specifying the shape and dimension of the TypedArray. They convienently have a constructor for each of the 10 numerics types that Futhark provides. TypedArrays have a number of advantages.
...
\begin{listing}[H] 
        \inputminted[fontsize=\small,baselinestretch=0.5,linenos]{C}{code/compiler/api_examples/example.html}
        \caption{64 bit multiplication with 128 bit casting}
        \label{lst:int128}    
\end{listing} 
