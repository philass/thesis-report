...
%A large fraction of today's software runs in the browser, and the underlying hardware of the laptops, tablets, and mobile phones that run these browsers have increasingly parallel computing capabilities with GPUs and multicore CPUs. 
...
%by compiling the functional data-parallel programming language Futhark to WASM and threaded WASM for efficient execution in web browsers on multicore CPUs.
...
%Edge Computing 
    % Doing network computation at 'local' nodes to increase response times over cloud computing
    % Especially wrt. data intensive applications, e.g., IoT/Smart Homes/Smart Grids/ 
    
    % Example Fastly supports web assemply for the security of sand-boxed execution environment of the virtual machime
    
%Dfinity 
    % Secure blockchain based distributed infrastructure for second coming of the internet
...
\begin{verbatim}
emcc concat.c -o concat.js -s MODULARIZE \ -s EXPORTED_FUNCTIONS=[_malloc,_free,_concat]
\end{verbatim}
...
\lstinputlisting[language=Haskell]{code/compiler/codegen/GenericWASM.hs}
